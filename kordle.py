...
class KordleAlarm(Plugin):
    ...
    def add_alarm(self, message: Message, hour: int, minute: int):
        if self.schedule.get_jobs().__len__() == 1:
            ...
            self.driver.direct_message(message.user_id, "꼬들 알림이 매일 `%d:%2d`에 전달됩니다." % (int(hour), int(minute)))
            ...
            # 알림 정보 저장
            job = self.schedule.get_jobs()[0]
            alarm = Alarm(message.sender_name, message.user_id, job, "mon-sun", "%d:%2d" % (int(hour), int(minute)))
