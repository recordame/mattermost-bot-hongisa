...
class WeatherAlarm(Plugin):
    ...
    def add_alarm(self, message: Message, loc: str, hour1: int, hour2: int):
        if self.schedule.get_jobs().__len__() == 1:
            ...
            self.schedule.add_job(func=lambda: self.driver.create_post(constant.CH_NOTIFICATIONS_ID, generate_msg(loc)),
                                  trigger='cron',
                                  day_of_week='mon-sun',
            ...
            # 알림 정보 저장
            job = self.schedule.get_jobs()[0]
            alarm = Alarm(message.sender_name, message.user_id, job, "mon-sun", "%d, %d" % (int(hour1), int(hour2)))
    ...
    def cancel_alarm(self, message: Message):
        ...
        job = self.schedule.get_jobs()[0]
        alarm: Alarm = constant.ALARMS.get(job.id)
        self.driver.direct_message(alarm.creater_id, "등록하신 날씨 알림이 %s님에 의해 삭제되었습니다. \n\n"
